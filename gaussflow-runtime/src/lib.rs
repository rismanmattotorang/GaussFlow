...
pub async fn execute(dag: TypeSafeDag, input: Value) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let cpu_sem = Arc::new(Semaphore::new(dag.settings.concurrency.unwrap_or(num_cpus::get() as u32) as usize));
    ...
    while let Some(nx) = topo.next(&dag.graph) {
        ...
        let predecessors = dag.graph.neighbors_directed(nx, petgraph::Direction::Incoming);
        ...
        let permit_sem = if resources.map_or(false, |r| r.gpu_count > 0) {
            gpu_sem.clone()
        } else {
            cpu_sem.clone()
        };
        ...
        let task = tokio::spawn(async move {
            ...
            loop {
                let timeout_ms = n_ref.resources.as_ref().map(|r| r.timeout_ms).unwrap_or(60_000);
                let exec_res = tokio::time::timeout(Duration::from_millis(timeout_ms), handler.execute(&n_ref, merged_input.clone())).await;
                match exec_res {
                    ...
                    Err(_) => {
                        // timeout elapsed
                        break Err(Box::new(std::io::Error::new(std::io::ErrorKind::TimedOut, format!("DAG Timeout: {}", n_ref.id.clone()))));
                    }
                }
            }
        });
    ...
    db.query("UPDATE type::thing('run', $id) SET status='finished', finished=time::now(), output=$out")
        .bind(("id", run_id.clone()))
