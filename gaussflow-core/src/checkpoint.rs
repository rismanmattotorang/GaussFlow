...
pub trait CheckpointStore: Send + Sync {
    ...
    async fn load_latest_checkpoint(&self, workflow_id: &str) -> Result<Option<Checkpoint>, DagError>;
...
impl CheckpointStore for FileCheckpointStore {
                    ...
                    if let Some(ext) = entry.path().extension() {
                        if ext == "json" {
                            if let Ok(content) = tokio::fs::read(entry.path()).await {
                                if let Ok(checkpoint) = serde_json::from_slice::<Checkpoint>(&content) {
    ...
    async fn delete_checkpoint(&self, checkpoint_id: &str) -> Result<(), DagError> {
        ...
        let path = self.base_path.join("workflow_id").join(format!("{}.json", checkpoint_id));
        if path.exists() {
...
mod tests {
    ...
    async fn test_file_checkpoint_store() {
        ...
        let loaded = store.load_latest_checkpoint("workflow_id").await.unwrap().unwrap();
        ...
        assert!(store.load_latest_checkpoint("workflow_id").await.unwrap().is_none());
    }
}
