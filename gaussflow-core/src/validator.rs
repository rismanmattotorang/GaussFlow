use crate::dag::{DagNode, DagEdge, TypeSafeDag, DagValidationError};
...
pub trait DagValidator: Debug + Send + Sync + 'static {
    ...
    fn validate(&self, dag: &TypeSafeDag<Self::Node, Self::Edge>) -> Result<(), DagValidationError>;
    ...
    fn validate_dependencies(
        ...
        for dep in node.dependencies() {
            if dag.get_node(&dep).is_none() {
                return Err(DagValidationError::MissingDependencies(format!("Missing dependency: {}", dep)));
            }
        }
        Ok(())
    }
    ...
    fn validate(&self, dag: &TypeSafeDag<N, E>) -> Result<(), DagValidationError> {
        ...
        for node_idx in dag.graph.node_indices() {
            if !visited.contains(&node_idx) {
                if self.detect_cycle(dag, node_idx, &mut visited, &mut rec_stack) {
