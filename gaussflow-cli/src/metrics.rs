...
impl MetricsManager {
    ...
    pub async fn record_workflow_completion(
        ...
        let total_completed = performance.workflow_metrics.completed_workflows
            + performance.workflow_metrics.failed_workflows;
        if total_completed > 0 {
            let current_avg = performance.workflow_metrics.avg_execution_time;
            let new_avg =
                (current_avg * (total_completed - 1) as u32 + duration) / total_completed as u32;
    ...
    pub async fn record_network_metrics(
        &self,
        bytes_sent: u64,
        bytes_received: u64,
        connections: u32,
    ...
    async fn collect_system_metrics(&self) -> Result<()> {
        ...
        let _total_memory = memory_info.total_memory() * 1024; // Convert to bytes
    ...
    async fn update_throughput(&self) -> Result<()> {
        ...
        if uptime.as_secs() > 0 {
            performance.workflow_metrics.throughput =
                (performance.workflow_metrics.completed_workflows
                    + performance.workflow_metrics.failed_workflows) as f64
...
impl std::fmt::Display for MetricsSummary {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "Memory Usage: {} MB",
            self.performance.memory_usage / 1024 / 1024
        )?;
        writeln!(
            f,
            "Total Workflows: {}",
            self.performance.workflow_metrics.total_workflows
        )?;
        writeln!(
            f,
            "Running Workflows: {}",
            self.performance.workflow_metrics.running_workflows
        )?;
        writeln!(
            f,
            "Completed Workflows: {}",
            self.performance.workflow_metrics.completed_workflows
        )?;
        writeln!(
            f,
            "Failed Workflows: {}",
            self.performance.workflow_metrics.failed_workflows
        )?;
        writeln!(
            f,
            "Throughput: {:.2} workflows/sec",
            self.performance.workflow_metrics.throughput
        )?;
...
mod tests {
    ...
    async fn test_metrics_manager_basic() {
        ...
        manager
            .record_workflow_start("test_workflow")
            .await
            .unwrap();
        manager
            .record_workflow_completion("test_workflow", Duration::from_secs(5), true)
            .await
            .unwrap();
        ...
        assert!(
            summary.uptime < Duration::from_secs(3600),
            "uptime should be a recent measurement"
        );
